...
categs = {'std','pca','template','template_pca'};
categ_names = {'Standard','PCA','Template','Both'};

% categs = {'std','pca','template'};
% categ_names = {'Standard','PCA','Template'};
...
for ampind = 1:length(categs)
    ...
    if strcmp(categs{ampind},'std')
        rem_art_pca = false;
    elseif strcmp(categs{ampind},'pca')
        rem_art_pca = true;
        mep_settings.art_type = 'emg';  
        mep_settings.art_chans = 'facial'; 
    elseif strcmp(categs{ampind},'template')
        mep_settings.art_type = 'template';  
    elseif strcmp(categs{ampind},'template_pca')
        mep_settings.art_type = 'template_pca';
    end
