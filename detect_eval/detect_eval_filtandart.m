...
% this version: PCA vs. template-based artifact rejection

categs = {'std','hp','notch','both','tmp_only','tmp_notch','tmp_hp','all'};
categ_names = {'Standard','High-pass','Notch','Both','Template Only','Template+Notch','Template+HP','All'};
...
f_hp = 20;
[b_high, a_high] = butter(2,[f_hp]/22000,'high');
...
for ampind = 1:length(categs)
    ...
    %{'std','hp','notch','both','tmp_only','all'}
    if strcmp(categs{ampind},'std')
        rem_art_pca = false;
    elseif strcmp(categs{ampind},'hp')
        filt_hp = true;
    elseif strcmp(categs{ampind},'notch')
        filt_hp = false;
        use_tdnotch = true;
    elseif strcmp(categs{ampind},'both')
        filt_hp = true;
        use_tdnotch = true;
    elseif strcmp(categs{ampind},'tmp_only')
        filt_hp = false; use_tdnotch = false;
        rem_art_pca = true;
    elseif strcmp(categs{ampind},'tmp_hp')
        filt_hp = true; use_tdnotch = false;
    elseif strcmp(categs{ampind},'tmp_notch')
        filt_hp = false; use_tdnotch = true;
    elseif strcmp(categs{ampind},'all')
        filt_hp = true; use_tdnotch = true;
    end
        ...
        for kp = 1:length(emg_mapping_all{pt_ind})
            ...
            for ko = 1:n_trials
                ...
                if rem_art_pca
                    ...
                    if filt_hp
                        for tempch = 1:length(mep_means)
                            mep_means{tempch}(40:end) = filtfilt(b_high, a_high, mep_means{tempch}(40:end));
                        end
                    end
                    if filt_lp
                        for tempch = 1:length(mep_means)
                            mep_means{tempch}(40:end) = filtfilt(b_low, a_low, mep_means{tempch}(40:end));
                        end
                    end
                    if use_frnotch
                        for tempch = 1:length(mep_means)
                            mep_means{tempch}(40:end) = filtfilt(b_notch, a_notch, mep_means{tempch}(40:end));
                        end
                    end
                    if use_tdnotch
                        for tempch = 1:length(mep_means)
                            mep_means{tempch} = td_notch(mep_means{tempch},tt,60,fs);
                        end
                    %[temptrace, yest, A, phi] = td_notch(temptrace,tt,60,fs);
                    end
