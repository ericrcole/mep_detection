...
% this version: different methods for scaling/fitting a fit template
% obtained via PCA to individual mEP examples 
% (orthogonal projection, median, percentile, PCA reconstruction without first PC)

categs = {'std','proj','median','pctl','recon'};
categ_names = {'Standard','Projection','Median','Percentile','Reconstructed'};
...
for ampind = 1:length(categs)
    ...
    if strcmp(categs{ampind},'std')
        rem_art_pca = false;
    elseif strcmp(categs{ampind},'proj')
        rem_art_pca = true;
        mep_settings.fit_type = 'proj';
    elseif strcmp(categs{ampind},'median')
        mep_settings.fit_type = 'median';
    elseif strcmp(categs{ampind},'pctl')
        mep_settings.fit_type = 'pctl';
    elseif strcmp(categs{ampind},'recon')
