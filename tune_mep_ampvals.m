%tune_MEP_outliers.m
%
% examine data to determine threshold for automatically excluding
% high-magnitude detected mEP peaks
%
%also generate final distributions for amplitude normalization and
%latencies for detection
...
all_amps_by_muscle_raw = cell(size(emg_keys));
...
for kk = 1:length(ep_struct)
    ...
    amps_detect = ep_struct(kk).mep.(mode).amps_detect;
...
%             end
            ...
            if ~isempty(amps_detect_z{km,kl})
                ...
                tempamp_raw = max(amps_detect{km,kl})';

                all_amps_by_muscle{key_ind} = [all_amps_by_muscle{key_ind}; tempamp];
                all_amps_by_muscle_raw{key_ind} = [all_amps_by_muscle_raw{key_ind}; tempamp_raw];
...
for kk = categ_inds
    ...
    boxchart(ones(size(temp))*tempind,temp,'MarkerStyle','none');
    hold on
    %scatter(ones(size(temp))*kk+randn(size(temp))*0.1, temp, 20, [0.5 0.5 0.5],'filled')
    tempind = tempind + 1;
end
...
if save_ltc
    ...
    save('MEP_latencies_opt_120624.mat','latencies_saved')
end
